...
import java.sql.*;
public class WardenReport extends JFrame
{
	JPanel pnl_second;
	JLabel lab_1;
	JButton but_prisoner, but_home, but_warden, but_prisonerReport, but_wardenReport;
	...
	String lab[]={"Id","Name","Rank","DTJOIN","MJOIN","YJOIN","DRETIRE","MRETIRE","YRETIRE","Department", "Address"};	
	int xcod=10, ycod=30;
	public WardenReport()
	{
		setSize(1000,500);
		...
		try
		{
			...
			ResultSet rs=stmt.executeQuery("Select *from WARDENDB ");
			while(rs.next())
			{
				xcod=10;
				for(int i=0;i<10;i++)
