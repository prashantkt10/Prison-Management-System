...
import javax.swing.*;
import java.sql.*;
class UpdateWarden extends JFrame implements ActionListener
{
	...
	public UpdateWarden()
	{
		...
		footer=new JPanel();
		center=new JPanel();
		lblh=new JLabel("____Update Warden____");
	...
	public void ItemsAdd()
	{
		...
		cmbcd.setBounds(330,235, 80,20);
		txtaddress.setBounds(330,275, 165,100);
		btupdate.setBounds(120,415, 80,40);
	...
	void clear()
	{
		...
		cmbcd.setSelectedIndex(0);
	}

	public static void main(String args[])
	{
		UpdateWarden aw=new UpdateWarden();
		aw.ItemsAdd();
	...
	public void Update()
	{
		...
		try
		{
			...
			String sql ="UPDATE WARDENDB set NM= '"+name+"', RANK= '"+rank+"', DOJ= '"+doj+"', MOJ= '"+moj+"', YOJ= '"+yoj+"', DOR= '"+dor+"', MOR='"+mor+"', YOR= '"+yor+"', CR_DES= '"+cd+"', ADDRESS= '"+address+"' WHERE UD= "+id;
			st.execute(sql);
